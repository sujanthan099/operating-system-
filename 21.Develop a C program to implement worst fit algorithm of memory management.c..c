...
// Function to find the worst fit block for a given process size
int findWorstFitBlock(struct Block memoryBlocks[], int numBlocks, int processSize) {
    int index = -1;
    int maxSize = -1;
...
void allocateMemory(struct Block memoryBlocks[], int numBlocks, int processSize, int processId) {
    int blockIndex = findWorstFitBlock(memoryBlocks, numBlocks, processSize);
    if (blockIndex != -1) {
