...
// Function to find the best fit block for a given process size
int findBestFitBlock(struct Block memoryBlocks[], int numBlocks, int processSize) {
    int index = -1;
    int minSize = -1;
    for (int i = 0; i < numBlocks; i++) {
        if (!memoryBlocks[i].isAllocated && memoryBlocks[i].size >= processSize) {
            if (minSize == -1 || memoryBlocks[i].size < minSize) {
...
void allocateMemory(struct Block memoryBlocks[], int numBlocks, int processSize, int processId) {
    int blockIndex = findBestFitBlock(memoryBlocks, numBlocks, processSize);
    if (blockIndex != -1) {
